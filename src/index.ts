...
class RobloxStudioLauncher {
  ...
  private async launchWindows(): Promise<void> {
    return new Promise((resolve, reject) => {
      const child = spawn("cmd", ["/c", "start", '""', this.protocolUrl], {
        detached: true,
        stdio: "ignore",
      });
...
async function main() {
  try {
    ...
    if (!values["place-id"] || !values["universe-id"]) {
      console.error("❌ Error: --place-id and --universe-id are required");
    ...
    if (!values["multiple-process"]) {
      ...
      if (hasExistingProcesses) {
        ...
        if (!noLogs) {
          console.log("💡 Use --multiple-process to allow multiple instances");
        }
